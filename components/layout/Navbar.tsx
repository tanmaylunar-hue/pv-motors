    ...
    <>
      ...
      >
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:h-20 lg:px-8">
          ...
          <button
            type="button"
            className="min-h-[48px] min-w-[48px] flex items-center justify-center p-2 text-muted transition-colors hover:text-foreground lg:hidden"
      ...
      <div className={cn("fixed inset-0 z-[9999] lg:hidden transition-all duration-300", mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}>
        {/* Backdrop Blur Overlay */}
        <div
          className={cn("fixed inset-0 bg-black/40 backdrop-blur-sm transition-opacity duration-300 cursor-pointer", mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none")}
          onClick={() => setMobileOpen(false)}
        />

        {/* Sliding Drawer Container */}
        <div className={cn("fixed right-0 top-0 bottom-0 z-[9999] w-[85%] max-w-[320px] bg-background border-l border-border p-6 shadow-2xl flex flex-col justify-between transition-transform duration-300 ease-out", mobileOpen ? "translate-x-0" : "translate-x-full")}>
          <div>
            {/* Header */}
            <div className="flex items-center justify-between pb-5 border-b border-border/55">
              ...
              <button
                type="button"
                className="min-h-[48px] min-w-[48px] flex items-center justify-center p-1.5 text-muted hover:text-foreground border border-border rounded-full hover:bg-surface-elevated transition-colors"
                onClick={() => setMobileOpen(false)}
            ...
            <div className="mt-8 space-y-1">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  className={cn(
                    "block px-4 py-4 text-[11px] sm:text-xs font-semibold uppercase tracking-[0.15em] sm:tracking-[0.18em] border border-transparent rounded-sm transition-all duration-200",
                    isLinkActive(link.href)
          ...
          <div className="pt-6 border-t border-border/55 space-y-4">
            ...
            <Button
              ...
              className="w-full text-xs py-3.5 uppercase tracking-widest min-h-[48px]"
              size="sm"
            >
              Book Ride
            </Button>
